...
def build_previous_context(outputs: dict, current_index: int) -> str:
    """
    يجمع كل مخرجات العقول السابقة
    بحيث كل عقل يعرف الرحلة كاملة،
    وليس فقط مخرج العقل الذي قبله.
    """
...
"""
    ...
    return trim_text(full_context, max_chars=50000)
...
def build_prompt(
    ...
    values = {
        ...
        "ALL_PREVIOUS_OUTPUTS": previous_context,
...
if st.session_state.outputs and not run:
    ...
    st.info(
        f"فيه {len(st.session_state.outputs)} مخرجات محفوظة. "
        "اضغط تشغيل عشان تكمل من آخر عقل."
    )
