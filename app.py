...
Google الرسمي Grounding)، وإمكانية لصق "سكريبت مرجعي" (بأي لغة أو لهجة)
عشان السكريبت الجديد يتكتب بنفس روحه وإيقاعه، مهما كان الموضوع مختلف.
"""
...
MODEL_OPTIONS = ["gemini-3.7-flash", "gemini-3.6-flash", "gemini-3.5-flash"]
...
def generate_script(api_key: str, model: str, topic: str, audience: str,
    ...
    config = types.GenerateContentConfig(
        system_instruction=build_system_instruction(
            research_mode, duration_min, reference_script=reference_script,
        ),
        tools=tools,
        max_output_tokens=16000,
        # temperature و top_p مرفوعين عن القيمة الافتراضية عشان يقلل
        # تشابه السكريبتات بين مرة والتانية لنفس الموضوع أو موضوع قريب.
        temperature=1.15,
        top_p=0.95,
    )
...
st.caption("سكريبتات بالعامية المصرية، بأسلوب ثابت أو بأسلوب سكريبت تختاره انت، جاهزة للتسجيل.")
...
with st.expander("✍️ سكريبت مرجعي (اختياري) — يخلي السكريبت الجديد يتكتب بنفس روحه"):
    st.caption(
        "الصق هنا سكريبت فيديو كامل عجبك أسلوبه في كتابة الهوك والشرح - مش لازم "
        "يكون بنفس الموضوع، ومش لازم يكون بالعامية المصرية (ممكن يكون إنجليزي أو "
        "سعودي أو أي لهجة تانية). النظام هياخد منه الإيقاع والبنية بس، وهيكتب "
        "السكريبت الجديد دايمًا بالعامية المصرية. الخانة دي مش بتتحفظ - كل ما "
        "تسيبها فاضية، التطبيق هيرجع للأسلوب الافتراضي بتاعه."
    )
    reference_script = st.text_area(
        "الصق السكريبت المرجعي هنا",
        height=220,
        placeholder="مثال: سكريبت فيديو عن الخوف عجبك أسلوبه، وعايز فيديو جديد عن موضوع تاني تمامًا بنفس الروح دي...",
        key="reference_script_input",
    )
...
if generate_clicked:
    ...
    if not api_key:
        ...
        if script_text:
            ...
            st.session_state["last_used_reference"] = bool(reference_script.strip())
...
if st.session_state.get("last_script"):
    ...
    caption_extra = " — بأسلوب السكريبت المرجعي اللي لصقته" if st.session_state.get("last_used_reference") else ""
    st.caption(f"عدد الكلمات: {wc:,} — المدة التقريبية: {est_minutes} دقيقة{caption_extra}")
