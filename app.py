...
from config import APP_TITLE, APP_TAGLINE, MIN_DURATION_MINUTES, MAX_DURATION_MINUTES, AppConfig, DEFAULT_MODEL, FALLBACK_MODEL, MODEL_OPTIONS, get_secret
...
def init():
    ...
    st.session_state.setdefault("model", get_secret("GEMINI_MODEL", DEFAULT_MODEL))
    ...
    with st.sidebar:
        ...
        for item in ["Dashboard", "Projects", "Voice DNA", "Settings"]:
            if st.button(item, use_container_width=True, type="secondary" if st.session_state.page != item else "primary"):
                ...
                st.rerun()
        st.divider()
        labels = [name for name, _ in MODEL_OPTIONS]
        values = [value for _, value in MODEL_OPTIONS]
        current_model = st.session_state.get("model", DEFAULT_MODEL)
        current_index = values.index(current_model) if current_model in values else values.index(DEFAULT_MODEL)
        chosen_label = st.selectbox("موديل Gemini", labels, index=current_index, key="model_selector")
        st.session_state.model = values[labels.index(chosen_label)]
        st.caption(f"المستخدم في الـWorkflow: `{st.session_state.model}`")
        st.caption("Workflow")
...
def workspace():
    ...
    with tabs[1]:
        ...
        if st.button("ابنِ Voice DNA"):
            ...
            if not samples: st.warning("أضف عينات أولًا.")
                ...
                try:
                    ai = GeminiClient(model=st.session_state.get("model", DEFAULT_MODEL))
    ...
    with tabs[2]:
        ...
        b1, b2 = st.columns(2)
        run_resume = b1.button("استكمال من آخر مرحلة", type="primary", use_container_width=True)
        run_restart = b2.button("إعادة التشغيل من البداية", use_container_width=True)
        if run_resume or run_restart:
            try:
                ai = GeminiClient(model=st.session_state.get("model", DEFAULT_MODEL), web_research=use_web)
                pipe = Pipeline(ai)
                bar = st.progress(0)
                label = st.empty()
                def progress(p, stage):
                    bar.progress(p); label.info(f"يعمل الآن: {stage}")
                pipe.run(state, use_web=use_web, progress=progress, on_stage=lambda s, _: save_state(s), resume=run_resume and not run_restart)
                save_state(state)
                st.success("خلص الـworkflow وتم حفظ المشروع.")
            except Exception as e:
                st.error("حصل تعذر مؤقت في خدمة Gemini. المشروع نفسه لم يضِع، وآخر مرحلة ناجحة محفوظة في قاعدة البيانات.")
                with st.expander("التفاصيل التقنية"):
                    st.exception(e)
                st.info("اضغط استكمال من آخر مرحلة بدل إعادة المشروع كله؛ النظام هيبدأ من أول مرحلة لم تُحفظ.")
    with tabs[3]:
...
def settings():
    ...
    current_model = st.session_state.get("model", DEFAULT_MODEL)
    st.write(f"الموديل الحالي: **{current_model}**")
    st.write(f"Fallback الحالي: **{FALLBACK_MODEL}**")
