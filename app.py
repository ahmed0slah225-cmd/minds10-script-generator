...
with st.sidebar:
    ...
    with st.expander("📖 قواعد أسلوب القناة (SCRIPT_RULES.md)"):
        ...
        if "rules_editor" not in st.session_state:
            st.session_state["rules_editor"] = get_script_rules()
        rules_editable = st.text_area("محتوى الملف", height=300, key="rules_editor")
    ...
    if st.button("🔄 ابدأ مشروع جديد", use_container_width=True):
        keep = {"manual_api_key"}  # سيب مفتاح الـ API زي ما هو
...
if stage == 1:
    ...
    st.caption(
        ...
        "نصوص فيديوهات مرجعية، ملاحظاتك الخاصة). لو رفعت سكريبتات سابقة "
        "بتاعتك ونوّعتها كـ\"عينة من أسلوبك الشخصي\"، الكاتب هيحاول يقلّد "
        "إيقاعك ومفرداتك بدل ما يكتب بصوت عام."
    )
    ...
    with st.expander("✏️ أضف ملاحظة أو نص يدويًا"):
        ...
        if st.button("➕ أضف كمصدر"):
            if note_content.strip():
                ...
                # نفضّي مربعات الإدخال بعد الإضافة (لازم تتمسح من session_state
                # مباشرة قبل الـ Rerun، لأنها ودجتس ليها key بيتذكرها بنفس
                # المشكلة اللي حلّيناها في مربعات كتابة الأجزاء).
...
# =============================== المرحلة ٢: استخراج المعرفة =============================== #
    ...
    if extraction:
        ...
        deeper_angles = extraction.get("deeper_angles", [])
        if deeper_angles:
            st.markdown("**🧠 زوايا تفكير أعمق (عشان السكريبت مايبقاش عام):**")
            for a in deeper_angles:
                st.markdown(f"- **{a.get('framework', '')}**: {a.get('insight', '')}")
...
# =============================== المرحلة ٥: كتابة السكريبت جزء جزء =============================== #
    ...
    for section in writable_sections:
        ...
        # الودجت بيتحكم في قيمته لوحده بمجرد ما ياخد key - فمينفعش نبعتله
        # value= كمان (Streamlit بيتجاهل الـ value الجديدة وقتها). الطريقة
        # الصح: نهيّئ session_state[widget_key] مرة واحدة بس، وبعد كده أي
        # تحديث (زي التوليد من جديد) بيتم على session_state[widget_key]
        # مباشرة من غير ما نمرر value= للودجت خالص.
        if widget_key not in st.session_state:
            st.session_state[widget_key] = st.session_state["sections_text"].get(key, "")

        with st.expander(f"### {section.get('title')}", expanded=key not in st.session_state["sections_text"]):
            st.caption(f"{section.get('goal', '')} — 🎯 الهدف: ~{section.get('target_words', '-')} كلمة")
            ...
            if c1.button(f"🎬 {'أعد كتابة' if current_text else 'اكتب'} هذا الجزء", key=f"write_{key}"):
                if require_api_key():
                    ...
                    voice_samples_text = st.session_state["kb"].get_voice_samples_text()
                    result, err = run_safely(
            ...
            edited = st.text_area("النص (تقدر تعدّله يدويًا)", height=350, key=widget_key)
            st.session_state["sections_text"][key] = edited
            if edited.strip():
                st.caption(f"📝 عدد الكلمات الحالي: {word_count(edited):,}")
...
# =============================== المرحلة ٦: المراجعة النهائية =============================== #
    ...
    if rc5.button("🤖 مراجعة الطبيعية", use_container_width=True) and require_api_key():
        result, err = run_safely(pipeline.review_naturalness, client, model, full_script)
        if err:
            st.error(err)
        else:
            reviews["naturalness"] = result
    ...
    if reviews.get("naturalness"):
        with st.expander("🤖 نتيجة مراجعة الطبيعية (ضد ريحة الـ AI)"):
            n = reviews["naturalness"]
            st.markdown(f"**المجموع: {n.get('total', '-')}/50 — {n.get('verdict', '')}**")
            st.json(n)
