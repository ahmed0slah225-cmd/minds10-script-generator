...
def _is_truncated(response) -> bool:
    return "MAX_TOKEN" in _get_finish_reason(response).upper()


def call_text(client, model: str, system_instruction: str, user_prompt: str,
              max_output_tokens: int = 8000, max_attempts: int = 3, max_continuations: int = 6) -> str:
    """
    بينادي الموديل ويرجع نص عادي (مش JSON). لو الموديل قطع الكلام لأنه
    وصل لحد max_output_tokens (بدل ما يخلّص الفكرة طبيعي)، البرنامج
    بيبعت طلب "كمّل من حيث ما وقفت" تلقائيًا لحد ما يخلّص أو يوصل
    max_continuations، عشان محدش يقف في نص كلمة أبدًا.
    """
    ...
    for round_i in range(max_continuations + 1):
        ...
        if _is_truncated(response) and round_i < max_continuations:
            current_prompt = (
                f"{user_prompt}\n\n---\n"
                f"ده اللي اتكتب لحد دلوقتي، وقف فجأة في نص الكلام:\n\n{accumulated}\n\n---\n"
                "كمّل الكتابة بالظبط من حيث ما وقفت (حتى لو وقف في نص كلمة)، "
                "من غير ما تعيد أي جزء سبق كتابته، ومن غير أي مقدمة أو تعليق - "
                "اكمل النص مباشرة."
            )
            continue
        break
