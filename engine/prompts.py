...

{
  ...
  "deeper_angles": [{"framework": "اسم زاوية التفكير", "insight": "الفكرة الجديدة/غير المتوقعة"}]
}

لو مفيش مصادر كفاية لموضوع معين، سيب القوائم فاضية ومتخترعش حاجة.

خاص بـ deeper_angles: دي مش تلخيص للمصادر - دي زوايا تفكير أعمق وأصلية
تخلي الفيديو مش عام أو متوقع، جرّب على الأقل ٢-٣ من الزوايا دي (لو
مناسبة للموضوع):
- "عكس الفكرة": لو قلبنا الفرضية الأساسية، هل تفضل صح؟ إيه اللي هيتغيّر؟
- "الدفاع عن الرأي المضاد": إيه أقوى حجة ممكن حد يقولها ضد الفكرة دي،
  ورد عليها بشكل منصف؟
- "الأثر بعيد المدى": لو الفكرة دي صح، إيه النتيجة غير المباشرة بعد سنين
  اللي محدش بيفكر فيها عادة؟
- "تشبيه جديد": تشبيه أو استعارة من مجال مختلف تمامًا (مش المتداول) يوضح
  الفكرة بشكل يخلي المشاهد يفتكرها.
هدف الحقل ده إنه يديك مادة لجزء "المفاجأة" أو "التفسير" في الهيكل، بدل
ما يبقى السكريبت مجرد سرد عام للمعلومات المعروفة.
""".strip()
...
def build_section_prompt(section: dict, topic: str, rules_text: str, extraction: dict,
                          previous_sections_text: str, chosen_hook: str, voice_samples_text: str = ""):
    target_words = section.get('target_words', 150)
    min_words = int(target_words * 0.9)
    voice_block = ""
    if voice_samples_text.strip():
        voice_block = f"""

دليل الصوت الشخصي (عينات من أسلوب صاحب القناة نفسه): قلّد إيقاع الجمل،
المفردات المفضّلة، والطريقة اللي بيبني بيها كلامه من العينات دي - من
غير ما تنسخ نفس الجمل حرفيًا:
{voice_samples_text}
"""
    system = _base_system(rules_text) + f"""
...
والمقدمة والخاتمة، ## لباقي الأجزاء)، بحجم لا يقل عن {min_words} كلمة
وحوالي {target_words} كلمة (مفيش حد أقصى صارم - لو احتجت تفاصيل أو
أمثلة أكتر عشان توصل للعدد ده بجودة، اكتبها). ماتكتبش أي جزء تاني غير
ده، وماتكررش الهوك المُختار لو مش هو الجزء المطلوب.

مهم جدًا: الطول ده مقصود عشان الفيديو يوصل للمدة المطلوبة - لو حسيت
إن الفكرة هتخلص بسرعة قبل ما توصل للعدد المطلوب، وسّع الشرح بمثال
واقعي تاني، أو تفصيلة من القصة، أو تطبيق عملي إضافي يخدم نفس الفكرة -
من غير حشو أو تكرار لمعنى اتقال قبل كده، لكن برضه من غير ما تختصر
الفكرة في سطرين وتقفل الجزء بدري.
...
def build_naturalness_review_prompt(full_script_text: str):
    system = """
انت "مراجع الطبيعية" (Naturalness / Anti-AI-Slop Reviewer). مهمتك
الوحيدة إنك تدوّر على "ريحة الذكاء الاصطناعي" جوه السكريبت - المبالغات
الجاهزة، العبارات التمهيدية الفاضية، الخواتيم الجاهزة ("في النهاية")،
الشرطات المعترضة الزيادة، القوائم المصطنعة، الأسئلة البلاغية المكررة،
النسب لمصادر غامضة ("خبراء بيقولوا"). قيّم السكريبت من 1 إلى 10 على
كل بُعد من الأبعاد الخمسة دي:

- المباشرة (directness): بيوصل الفكرة على طول ولا بيلف حواليها؟
- الإيقاع (rhythm): طول الجمل متنوع ولا كله على نفس الوتيرة؟
- الثقة بالمشاهد (trust): بيحترم ذكاء المشاهد ولا بيشرح الواضح؟
- الطبيعية (authenticity): بيحس إنه مكتوب من إنسان بيتكلم فعلًا؟
- الكثافة (density): كل جملة فيها فايدة، ولا فيه حشو ممكن يتشال؟

رجّع JSON فقط:
{"scores": {"directness": 0, "rhythm": 0, "trust": 0, "authenticity": 0, "density": 0},
 "total": 0,
 "violations": [{"pattern": "اسم النمط", "excerpt": "المقطع من النص", "fix": "تعديل مقترح"}],
 "verdict": "جاهز" أو "محتاج إعادة صياغة"}

مجموع الدرجات (total) من 50. لو total أقل من 35، الـ verdict لازم
يبقى "محتاج إعادة صياغة".
""".strip()
    user = f"السكريبت الكامل:\n{full_script_text}"
    return system, user
