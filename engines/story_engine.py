from core.context import PipelineContext
from engines.base import BaseEngine
from engines.topic_engine import _extract_json
from providers.base import LLMProvider
from prompts.templates import STORY_PROMPT, SYSTEM_ANALYST


class StoryEngine(BaseEngine):
    name = "story"

    def run(self, ctx: PipelineContext, provider: LLMProvider) -> PipelineContext:
        topic = (ctx.topic_analysis or {}).get("topic", "")
        strategy = str(ctx.strategy or {})
        prompt = STORY_PROMPT.format(topic=topic, strategy=strategy)
        raw = self.call_llm(
            ctx, provider, prompt,
            system_instruction=SYSTEM_ANALYST,
            temperature=0.6,
            response_mime_type="application/json",
        )
        ctx.story = _extract_json(raw)
        return ctx