...
class ResearchEngine(LLMEngine):
    ...
    def task(self,ctx): return f'ابحث خارجيًا فقط عما تحتاجه الاستراتيجية. عمق البحث={ctx.settings.research_depth}. ابدأ بأسئلة المعرفة الناقصة. أخرج JSON يحتوي research_questions, findings, sources، مع provenance وURL ووقت الجمع وconfidence. لا تخترع روابط أو تحققًا.'
    def run(self,ctx,llm):
        if not ctx.settings.web_research:
            ctx.state.metadata['research_status']='disabled_by_user'; return
        r=llm.generate(skill_prompt(self.skill,self.task(ctx),self.context_text(ctx),ctx.state.voice_dna),model_label=ctx.settings.model_label,json_mode=True,web_search=True,temperature=self.temperature)
        d=parse_json(r.text); self.apply(ctx,d); ctx.record(self.name,model=r.model_id,run_id=r.run_id)
