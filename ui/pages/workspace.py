...
def render(repo, settings):
    ...
    with c2:
        st.checkbox(
            '🔎 البحث المباشر من الإنترنت',
            key='use_web_search',
            help='فعّلها فقط لو عايز Gemini يستخدم Google Search أثناء مرحلة البحث. لو غير مفعّلة، المشروع يشتغل بدون بحث مباشر من الإنترنت.'
        )
    if st.session_state['use_web_search']:
        st.info('البحث المباشر مفعّل: سيتم استخدام Google Search أثناء مرحلة البحث، وقد يتطلب الخطة المناسبة في Gemini API.')
    else:
        st.caption('البحث المباشر غير مفعّل — المشروع سيعمل اعتمادًا على المدخلات والمصادر التي رفعتها، بدون Google Search.')
